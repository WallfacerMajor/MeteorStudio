...
class WhiteBalanceWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        footer.pack_forget()
        output.pack_forget()
        footer.pack(in_=sidebar, side="bottom", fill="x", before=self.control_canvas, pady=5)
        output.pack(in_=sidebar, side="bottom", fill="x", before=self.control_canvas, pady=5)
        footer.lift()
        output.lift()
        ...
        from workspace_layout import stack_controls
        stack_controls(output, 220)
